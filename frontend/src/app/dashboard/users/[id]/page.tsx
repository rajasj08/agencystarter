          ...
          <AppCard className="max-w-2xl rounded-xl p-6">
            {user.deletedAt != null && (
              <div className="mb-4 rounded-md border border-amber-500/50 bg-amber-500/10 px-3 py-2 text-sm text-amber-700 dark:text-amber-400">
                This user has been deleted. You can restore them from the Edit page.
              </div>
            )}
            <dl className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <dd className="mt-1 space-y-1">
                  <UserStatusBadge status={user.deletedAt != null ? "DELETED" : user.status} />
                  {(user.status === "INVITED" || user.status === "PENDING_VERIFICATION") && user.deletedAt == null && (
                    <p className="text-xs text-text-secondary">This user has not completed account setup yet.</p>
                  )}
                </dd>
              </div>
              {user.deletedAt && (
                <div>
                  <dt className="text-sm font-medium text-text-secondary">Deleted at</dt>
                  <dd className="mt-1 text-sm text-text-primary">{formatDate(user.deletedAt)}</dd>
                </div>
              )}
              <div>
