...
export default function SuperadminPlanEditPage() {
  ...
  const [planUpdatedBy, setPlanUpdatedBy] = useState<EditorInfo | null>(null);
  ...
  useEffect(() => {
    ...
    getPlanById(planId)
      .then((plan) => {
        ...
        setPlanUpdatedAt(plan.updatedAt);
        setPlanUpdatedBy(plan.updatedBy ?? null);
    ...
    <div className="mx-auto max-w-[1200px]">
      <FormProviderWrapper form={form} onSubmit={handleSubmit} id="edit-plan-form">
        <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-xl font-semibold text-text-primary">Edit Plan</h1>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            <Card className="rounded-2xl shadow-sm">
              ...
              <CardContent className="space-y-4 p-6">
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  ...
                  <div>
                    <label className="mb-1 block text-sm font-medium text-text-primary">Plan Code</label>
          ...
          <div className="lg:col-span-4">
            <LastEditedSummary updatedAt={planUpdatedAt} updatedBy={planUpdatedBy} />
          </div>
        </div>
      </FormProviderWrapper>
    </div>
...
function FeatureToggleEdit({ name, label }: { name: Path<FormValues>; label: string }) {
